...
export function SharedFiles({ conversationId, partner, assets }: SharedFilesProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      <header className="sticky top-0 z-20 border-b border-black/5 bg-white/95 px-5 pb-4 pt-10 backdrop-blur">
        <Link href="/chats" className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-neutral-500">
          <ArrowLeft className="h-4 w-4" />
          Back to chats
        </Link>
        <div className="space-y-1">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-brand/70">Shared Cloud</p>
          <h1 className="text-3xl font-extrabold tracking-tight text-neutral-950">{partner.display_name}</h1>
          <p className="text-sm text-neutral-500">{assets.length} shared files</p>
        </div>
          ...
          {(["all", "image", "video", "document"] as const).map((item) => (
            <button
              ...
              className={cn(
                ...
                filter === item ? "bg-brand text-white" : "bg-neutral-100 text-neutral-500",
              )}
            >
      ...
      <main className="grid flex-1 grid-cols-2 gap-3 p-4 pb-24">
        {filteredAssets.length ? (
          filteredAssets.map((asset) => (
            <article key={asset.id} className="overflow-hidden rounded-[1.5rem] border border-black/5 bg-neutral-50">
              {asset.mime_type.startsWith("image/") ? (
                asset.signed_url ? (
                  <Image
                  ...
                  <div className="flex aspect-square items-center justify-center bg-neutral-100 text-xs font-semibold text-neutral-500">
                    Image unavailable
                  </div>
                )
              ) : asset.mime_type.startsWith("video/") ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex aspect-square items-center justify-center bg-neutral-200 text-neutral-700"
                >
              ...
              <div className="space-y-3 p-3">
                <div>
                  <p className="truncate text-sm font-semibold text-neutral-900">{asset.attachment_name}</p>
                  <p className="text-xs text-neutral-400">{formatTimestamp(asset.created_at)}</p>
                </div>
                <div className="flex items-center gap-3 text-xs font-semibold">
                  <a href={asset.signed_url ?? "#"} target="_blank" rel="noreferrer" className="text-brand">
                    Preview
                  </a>
                  <a href={asset.signed_url ?? "#"} target="_blank" rel="noreferrer" className="text-neutral-500">
                    Download
                  </a>
                  {asset.is_mine ? (
                    <button
          ...
          <div className="col-span-2 py-16 text-center">
            <p className="text-lg font-bold text-neutral-900">No files here yet</p>
            <p className="mt-2 text-sm text-neutral-500">Attachments from this conversation will appear here.</p>
          </div>
        )}
      </main>
